...
impl PreludeType {
    ...
    pub fn _to_type_name(&self, full_name_map: &FullNameMap) -> String{
        match self {
            PreludeType::NotPrelude(type_) => {
                api_util::_type_name(type_, full_name_map)
            },
    ...
    pub fn _is_final_type(&self) -> bool {
        match self {
            ...
            PreludeType::PreludeResult{..} | PreludeType::PreludeOption(..) => {
                false
            },
        }
    }
...
pub fn _prelude_type_need_special_dealing(type_: &clean::Type, full_name_map: &FullNameMap) -> bool{
...
impl _PreludeHelper {
    pub fn _from_call_type(call_type: &CallType) -> HashSet<_PreludeHelper> {
        match call_type {
            CallType::_DirectCall | CallType::_NotCompatible | CallType::_AsConvert(_)=> HashSet::new(),
            CallType::_BorrowedRef(inner_call_type) | CallType::_ConstRawPointer(inner_call_type,_) |
            CallType::_MutBorrowedRef(inner_call_type) | CallType::_MutRawPointer(inner_call_type,_) |
            CallType::_Deref(inner_call_type) | CallType::_ToOption(inner_call_type) |
            CallType::_ToResult(inner_call_type) | CallType::_UnsafeDeref(inner_call_type) => {
