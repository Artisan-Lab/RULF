...
impl GenericFunction {
    ...
    pub fn should_notate_return_type(&self) -> bool {
        let return_type_generics = self.api_function.output.as_ref().map_or(HashSet::new(), |output_ty| {
            get_generics_of_clean_type(&output_ty)
        });
        let input_type_generics = self.api_function.inputs.iter().fold(HashSet::new(), |mut former, ty| {
            former.extend(get_generics_of_clean_type(ty));
            former
        });
        return_type_generics.iter().any(|generic| {
            !input_type_generics.contains(generic)
        })
    }
