use crate::clean::{self, PrimitiveType, GetDefId};
...
pub fn _is_generic_type(ty: &clean::Type) -> bool {
    ...
    match ty {
        ...
        clean::Type::Slice(type_) | clean::Type::Array(type_,..) | 
        clean::Type::RawPointer(_, type_) | clean::Type::BorrowedRef{type_,..}=> {
...
pub fn _is_end_type(ty: &clean::Type, full_name_map : &FullNameMap) -> bool {
    match ty {
        ...
        clean::Type::Slice(inner) | clean::Type::Array(inner,..) 
...
pub fn _type_name(type_ : &clean::Type, full_name_map : &FullNameMap) -> String {
    ...
    match type_ {
        clean::Type::Primitive(primitive_type) => {
            primitive_type.as_str().to_string()
        },
        clean::Type::Generic(generic) => {
            generic.to_string()
        },
        clean::Type::BorrowedRef{type_,..} => {
            let inner_type = &**type_;
...
pub fn _same_type(output_type: &clean::Type, input_type: &clean::Type, hard_mode:bool, full_name_map : &FullNameMap)-> CallType {
    if hard_mode {
...
//hard_mode
pub fn _same_type_hard_mode(output_type: &clean::Type, input_type: &clean::Type, full_name_map : &FullNameMap) -> CallType {
    ...
    match output_type {
        ...
        clean::Type::Primitive(primitive_type) => {
            _same_type_primitive(primitive_type, input_type)
        },
        clean::Type::BareFunction(_bare_function) => {
            //TODO:有需要的时候在考虑
            CallType::_NotCompatible
        },
        clean::Type::Tuple(_inner_types) => {
            CallType::_NotCompatible
        },
        clean::Type::Slice(_inner_type) => {
            CallType::_NotCompatible
        },
        clean::Type::Array(_inner_type, _) => {
            CallType::_NotCompatible
        },
...
fn _same_type_resolved_path(output_type:&clean::Type, input_type: &clean::Type, full_name_map : &FullNameMap)->CallType{
...
fn _same_type_primitive(primitive_type: &PrimitiveType, input_type: &clean::Type)->CallType {
    match primitive_type {
        PrimitiveType::Isize | PrimitiveType::I8 | PrimitiveType::I16 | PrimitiveType::I32 | 
        ...
        PrimitiveType::U128 | PrimitiveType::F32 | PrimitiveType::F64 => {
            //数字类型
                    ...
                    match input_primitive_type {
                        ...
                        PrimitiveType::U128 | PrimitiveType::F32 | PrimitiveType::F64 => {
                            if output_primitive_type == input_primitive_type {
                                ...
                                return CallType::_AsConvert(input_primitive_type.as_str().to_string());
                            }
                        },
                        ...
                        PrimitiveType::Bool | PrimitiveType::Str | PrimitiveType::Slice | PrimitiveType::Array | 
                        PrimitiveType::Tuple | PrimitiveType::Unit | PrimitiveType::RawPointer | 
                        PrimitiveType::Reference | PrimitiveType::Fn | PrimitiveType::Never => {
                            return CallType::_NotCompatible;
                        }
                    }
                },
        ...
        PrimitiveType::Char => {
            match input_type {
                clean::Type::Primitive(inner_primitive_type)=> {
                    match inner_primitive_type {
                        PrimitiveType::Char => {
                            return CallType::_DirectCall;
                        },
                        PrimitiveType::U8 => {
...
//输出类型是RawPointer的情况
fn _same_type_raw_pointer(type_: &Box<clean::Type>, input_type: &clean::Type,full_name_map : &FullNameMap) -> CallType {
    let inner_type = &**type_;
    let inner_compatible = _same_type_hard_mode(inner_type, input_type, full_name_map);
    match inner_compatible {
        CallType::_NotCompatible => {return CallType::_NotCompatible;},
        _ => {
...
//输出类型是BorrowedRef的情况
fn _same_type_borrowed_ref(type_: &Box<clean::Type>, input_type: &clean::Type, full_name_map : &FullNameMap) -> CallType {
    let inner_type = &**type_;
    let inner_compatible = _same_type_hard_mode(inner_type, input_type, full_name_map);
    match inner_compatible {
        CallType::_NotCompatible => {return CallType::_NotCompatible;},
...
//处理输入类型是引用的情况
pub fn _borrowed_ref_in_same_type(mutability:&Mutability, type_: &Box<clean::Type>, output_type: &clean::Type, full_name_map : &FullNameMap) -> CallType {
    let inner_type = &**type_;
...
//处理输入类型是裸指针的情况
pub fn _raw_pointer_in_same_type(mutability:&Mutability, type_: &Box<clean::Type>, output_type: &clean::Type, full_name_map : &FullNameMap) -> CallType {
    let inner_type = &**type_;
    let inner_compatible = _same_type_hard_mode(output_type, inner_type, full_name_map);
    match &inner_compatible {
        CallType::_NotCompatible => {
        ...
        _ => {
            match mutability {
                Mutability::Mut => {
                    return CallType::_MutRawPointer(Box::new(inner_compatible.clone()), inner_type.clone());
                },
                Mutability::Not => {
                    return CallType::_ConstRawPointer(Box::new(inner_compatible.clone()), inner_type.clone());
                }
            }
        }
    }
...
pub fn _copy_type(type_:&clean::Type) -> bool{
    match type_ {
        ...
        clean::Type::BareFunction(_) | clean::Type::Never|clean::Type::Infer => {
            return false
        },
        clean::Type::Tuple(types) => {
...
pub fn _move_condition(input_type: &clean::Type, call_type:&CallType)-> bool {
    ...
    if !_copy_type(input_type) {
        match call_type {
            CallType::_DirectCall | CallType::_Deref(..) | CallType::_UnsafeDeref(..) |
            CallType::_UnwrapOption(..) | CallType::_UnwrapResult(..)=> {
                return true;
            },
            _=> {}
        }
    ...
    match  input_type_{
        clean::Type::BorrowedRef{mutability,..} | clean::Type::RawPointer(mutability,_) => {
            if let Mutability::Mut = *mutability {
                match call_type {
                    CallType::_DirectCall | CallType::_MutBorrowedRef(..) | CallType::_MutRawPointer(..) => {
                        return true;
                    },
                    _ => {}
                }
    ...
    match input_type {
        clean::Type::BorrowedRef{mutability,..} | clean::Type::RawPointer(mutability,_) => {
            if let Mutability::Not = *mutability {
                match call_type {
                    CallType::_DirectCall | CallType::_BorrowedRef(..) | CallType::_ConstRawPointer(..) => {
                        return true;
                    },
                    _ => {}
                }
...
pub fn _need_mut_tag(call_type: &CallType) -> bool{
    match call_type {
        CallType::_MutBorrowedRef(..) | CallType::_MutRawPointer(..) => {
            true
        },
        _ => {
            false
...
pub fn _resolved_path_equal_without_lifetime(ltype: &clean::Type, rtype: &clean::Type) -> bool {
    if let clean::Type::ResolvedPath{path:lpath,did:ldid,is_generic:lis_generic, ..} = ltype {
        if let clean::Type::ResolvedPath{path:rpath,did: rdid,is_generic: ris_generic, ..} = rtype {
...
fn new_segments_without_lifetime(old_path_segments:&Vec<clean::PathSegment>) -> Vec<clean::PathSegment> {
