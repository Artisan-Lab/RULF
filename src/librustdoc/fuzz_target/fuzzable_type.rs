...
impl FuzzableCallType {
    pub fn generate_fuzzable_type_and_call_type(&self) -> (FuzzableType, CallType) {
        ...
        match self {
            ...
            FuzzableCallType::Primitive(primitive) => (FuzzableType::Primitive(primitive.clone()), CallType::_DirectCall),
            ...
            FuzzableCallType::ConstRawPoiner(fuzzable_call_type_,type_) => {
                let (fuzzable_type, inner_call_type) = fuzzable_call_type_.generate_fuzzable_type_and_call_type();
                if let FuzzableType::NoFuzzable = fuzzable_type{
                    return (FuzzableType::NoFuzzable, CallType::_NotCompatible);
                }else if let CallType::_NotCompatible = inner_call_type {
                    return (FuzzableType::NoFuzzable, CallType::_NotCompatible);
                }
                return (fuzzable_type, CallType::_ConstRawPointer(Box::new(inner_call_type), type_.clone()));
            },
            FuzzableCallType::MutRawPoiner(fuzzable_call_type_,type_) => {
                ...
                return (fuzzable_type, CallType::_MutRawPointer(Box::new(inner_call_type), type_.clone()));
            },
            FuzzableCallType::BorrowedRef(type_) => {
                let inner_type = &**type_;
                //序列切片
                if let FuzzableCallType::Slice(slice_inner) = inner_type{
                    let (fuzzable_type, inner_call_type) = slice_inner.generate_fuzzable_type_and_call_type();
                    if let FuzzableType::NoFuzzable = fuzzable_type{
                        return (FuzzableType::NoFuzzable, CallType::_NotCompatible);
                    }else if let CallType::_NotCompatible = inner_call_type {
                        return (FuzzableType::NoFuzzable, CallType::_NotCompatible);
                    }
                    return (FuzzableType::RefSlice(Box::new(fuzzable_type)), CallType::_DirectCall);
                }
                //一般的引用
        ...
        match self {
            ...
            FuzzableType::Primitive(primitive_type) => {
                match primitive_type {
                    //TODO:Bool变量的长度是多少
                    clean::PrimitiveType::I8 | clean::PrimitiveType::U8 | clean::PrimitiveType::Bool => 1,
                    clean::PrimitiveType::I16 | clean::PrimitiveType::U16 => 2,
                    clean::PrimitiveType::I32 | clean::PrimitiveType::U32 | 
                    clean::PrimitiveType::Char| clean::PrimitiveType::F32 => 4,
                    //TODO:在我的64位机器上，isize,usize的宽度为8个字节
                    clean::PrimitiveType::I64 | clean::PrimitiveType::U64 | clean::PrimitiveType::F64 | 
                    clean::PrimitiveType::Usize | clean::PrimitiveType::Isize => 8,
                    clean::PrimitiveType::I128 | clean::PrimitiveType::U128 => 16,
                    _=>0,
                }
            },
            FuzzableType::RefSlice(inner_fuzzable) => {
                inner_fuzzable._min_length()
            },
    ...
    pub fn _to_type_string(&self) -> String{
        match self {
            FuzzableType::NoFuzzable => {
                "nofuzzable".to_string()
            },
            FuzzableType::Primitive(primitive) => {
...
pub fn fuzzable_call_type(ty_: &clean::Type, full_name_map : &FullNameMap) -> FuzzableCallType {
    match ty_ {
        clean::Type::ResolvedPath{..} => {
            ...
            match &prelude_type {
                PreludeType::NotPrelude(..) | PreludeType::PreludeResult{..} => FuzzableCallType::NoFuzzable,
        ...
        clean::Type::RawPointer(mutability, type_) => {
            ...
            match inner_fuzzable {
                ...
                _ => {
                    match mutability {
                        Mutability::Mut => {
                            return FuzzableCallType::MutRawPoiner(Box::new(inner_fuzzable), inner_type.clone());
                        },
                        Mutability::Not => {
                            return FuzzableCallType::ConstRawPoiner(Box::new(inner_fuzzable), inner_type.clone());
                        }
                    }
                }
            }
        ...
        clean::Type::BorrowedRef{lifetime,mutability,type_,..} => {
            ...
            if *inner_type == clean::Type::Primitive(PrimitiveType::Str) && *mutability == Mutability::Not{
