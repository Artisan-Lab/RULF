#![feature(int_log)]
#![feature(allocator_api)]

#[macro_use]
extern crate afl;
fn _to_u8(data:&[u8], index:usize)->u8 {
    data[index]
}

fn _to_u16(data:&[u8], index:usize)->u16 {
    let data0 = _to_u8(data, index) as u16;
    let data1 = _to_u8(data, index+1) as u16;
    data0 << 8 | data1
}

fn _to_slice<T>(data:&[u8], start_index: usize, end_index: usize)->&[T] {
    let data_slice = &data[start_index..end_index];
    let (_, shorts, _) = unsafe {data_slice.align_to::<T>()};
    shorts
}

fn test_function37(_param0 :tui::layout::Rect ,_param1 :&[char] ,_param2 :tui::layout::Rect) {
    let mut _local0 = tui::buffer::Buffer::empty(_param0);
    let _local1: tui::widgets::Table::<'_> = tui::widgets::Table::<'_>::new(_param1);
    let _local2_param2_helper1 = &mut (_local0);
    let _ = <tui::widgets::Table::<'_> as tui::widgets::Widget>::render(_local1, _param2, _local2_param2_helper1);
}

fn _read_data()-> Vec<u8> {
    use std::env;
    use std::process::exit;
    let args:Vec<String> = env::args().collect();
    if args.len() < 2 {
        println!("No crash filename provided");
        exit(-1);
    }
    use std::path::PathBuf;
    let crash_file_name = &args[1];
    let crash_path = PathBuf::from(crash_file_name);
    if !crash_path.is_file() {
        println!("Not a valid crash file");
        exit(-1);
    }
    use std::fs;
    let data =  fs::read(crash_path).unwrap();
    data
}

fn main() {
    let _content = _read_data();
    let data = &_content;
    println!("data = {:?}", data);
    println!("data len = {:?}", data.len());
    //actual body emit
    if data.len() < 20 {return;}
    let dynamic_length = (data.len() - 16) / 1;
    let _param0 = tui::layout::Rect{x: _to_u16(data, 0), y: _to_u16(data, 2), width: _to_u16(data, 4), height: _to_u16(data, 6)};
    let _param1 = _to_slice::<char>(data, 16 + 0 * dynamic_length, data.len());
    let _param2 = tui::layout::Rect{x: _to_u16(data, 8), y: _to_u16(data, 10), width: _to_u16(data, 12), height: _to_u16(data, 14)};
    test_function37(_param0 ,_param1 ,_param2);

}