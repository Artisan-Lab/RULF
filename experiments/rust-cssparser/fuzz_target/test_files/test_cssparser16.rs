#![feature(int_log)]
#![feature(allocator_api)]

#[macro_use]
extern crate afl;
fn _to_str(data:&[u8], start_index: usize, end_index: usize)->&str {
    let data_slice = &data[start_index..end_index];
    use std::str;
    match str::from_utf8(data_slice) {
        Ok(s)=>s,
        Err(_)=>{
            use std::process;
            process::exit(0);
        }
    }
}

fn _to_u8(data:&[u8], index:usize)->u8 {
    data[index]
}

fn _to_bool(data:&[u8], index: usize)->bool {
    let bool_value = _to_u8(data, index);
    if bool_value %2 == 0 {
        true
    } else {
        false
    }
}

fn _to_f32(data:&[u8], index: usize) -> f32 {
    let data_slice = &data[index..index+4];
    use std::convert::TryInto;
    let data_array:[u8;4] = data_slice.try_into().expect("slice with incorrect length");
    f32::from_le_bytes(data_array)
}

fn test_function16(_param0 :&str ,_param1 :f32 ,_param2 :bool) {
    let mut _local0 = <std::string::String as std::convert::From::<&str>>::from(_param0);
    let _local1_param0_helper1 = &mut (_local0);
    let mut _local1: cssparser::CssStringWriter::<'_, std::string::String> = cssparser::CssStringWriter::<'_, std::string::String>::new(_local1_param0_helper1);
    let _local2_param0_helper1 = &mut (_local1);
    let _local2_param1_helper1 = Some(_param1);
    let _: std::fmt::Result = cssparser::color::serialize_color_alpha(_local2_param0_helper1, _local2_param1_helper1, _param2);
}

fn main() {
    fuzz!(|data: &[u8]| {
        //actual body emit
        if data.len() < 6 {return;}
        let dynamic_length = (data.len() - 5) / 1;
        let _param0 = _to_str(data, 5 + 0 * dynamic_length, data.len());
        let _param1 = _to_f32(data, 0);
        let _param2 = _to_bool(data, 4);
        test_function16(_param0 ,_param1 ,_param2);
    });
}
