#![feature(int_log)]
#![feature(allocator_api)]

#[macro_use]
extern crate afl;
fn _unwrap_option<T>(_opt: Option<T>) -> T {
    match _opt {
        Some(_t) => _t,
        None => {
            use std::process;
            process::exit(0);
        }
    }
}

fn _to_str(data:&[u8], start_index: usize, end_index: usize)->&str {
    let data_slice = &data[start_index..end_index];
    use std::str;
    match str::from_utf8(data_slice) {
        Ok(s)=>s,
        Err(_)=>{
            use std::process;
            process::exit(0);
        }
    }
}

fn test_function22(_param0 :&str ,_param1 :&str) {
    let mut _local0 = <lofty::aac::AacFile as std::default::Default>::default();
    let _local1 = <lofty::id3::v2::Id3v2Tag as std::default::Default>::default();
    let _local2_param0_helper1 = &mut (_local0);
    let mut _local2 = lofty::aac::AacFile::set_id3v2(_local2_param0_helper1, _local1);
    let mut _local3 = lofty::iff::wav::RIFFInfoList::new();
    let _local4_param0_helper1 = &mut (_local3);
    let _local4 = lofty::iff::wav::RIFFInfoList::remove(_local4_param0_helper1, _param0);
    let mut _local5 = lofty::iff::wav::RIFFInfoList::new();
    let _local6_param0_helper1 = &mut (_local5);
    let _local6 = lofty::iff::wav::RIFFInfoList::remove(_local6_param0_helper1, _param1);
    let mut _local7_param0_helper1 = _unwrap_option(_local2);
    let _local7_param0_helper2 = &mut (_local7_param0_helper1);
    let _local7_param1_helper1 = _unwrap_option(_local4);
    let _local7_param2_helper1 = _unwrap_option(_local6);
    let _ = lofty::id3::v2::Id3v2Tag::insert_user_text(_local7_param0_helper2, _local7_param1_helper1, _local7_param2_helper1);
}

fn main() {
    fuzz!(|data: &[u8]| {
        //actual body emit
        if data.len() < 2 {return;}
        let dynamic_length = (data.len() - 0) / 2;
        let _param0 = _to_str(data, 0 + 0 * dynamic_length, 0 + 1 * dynamic_length);
        let _param1 = _to_str(data, 0 + 1 * dynamic_length, data.len());
        test_function22(_param0 ,_param1);
    });
}
